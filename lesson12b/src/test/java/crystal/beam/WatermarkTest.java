...
public class WatermarkTest implements Serializable {
  ...
  private static final Instant FIRST_EVENT_INSTANT = new Instant(0);
  private static final Instant ONE_SECOND_LATER_INSTANT = FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(1));
  private static final Instant TWO_SECONDS_LATER_INSTANT = FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(2));

  @Test
  public void should_show_unobservably_late_data() {
    Pipeline pipeline = BeamUtils.createPipeline("Unobservably late data");
    Coder<String> utfCoder = StringUtf8Coder.of();
    TestStream<String> lettersStream = TestStream.create(utfCoder).addElements(
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
            TimestampedValue.of("a", TWO_SECONDS_LATER_INSTANT)
        )
    ...
    IntervalWindow window1 = new IntervalWindow(FIRST_EVENT_INSTANT, FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(5)));
  ...
  public void should_discard_late_data_with_default_configuration() {
    Pipeline pipeline = BeamUtils.createPipeline("Discarded late data");
    Coder<String> utfCoder = StringUtf8Coder.of();
    TestStream<String> lettersStream = TestStream.create(utfCoder).addElements(
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
        ...
        .advanceWatermarkTo(FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(6)))
        .addElements(TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT))
        .advanceWatermarkToInfinity();
    Duration windowDuration = Duration.standardSeconds(5);
    ...
    IntervalWindow window1 = new IntervalWindow(FIRST_EVENT_INSTANT, FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(5)));
    PAssert.that(results).inFinalPane(window1).containsInAnyOrder("a=3"); // Why not 4 ??!!!
    pipeline.run().waitUntilFinish();
  }
  ...
  public void should_accept_late_data_with_allowed_lateness_and_accumulation_mode() {
    ...
    Pipeline pipeline = BeamUtils.createPipeline("Accepted late data with accepted previous panes");
    Coder<String> utfCoder = StringUtf8Coder.of();
    TestStream<String> lettersStream = TestStream.create(utfCoder).addElements(
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
        ...
        .advanceWatermarkTo(FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(6)))
        .addElements(TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT))
        .advanceWatermarkTo(FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(11)))
        .advanceWatermarkToInfinity();
    Duration windowDuration = Duration.standardSeconds(5);
    Window<String> window = Window.<String>into(FixedWindows.of(windowDuration))
  ...
  public void should_accept_late_data_but_discard_the_data_after_watermark() {
    ...
    Pipeline pipeline = BeamUtils.createPipeline("Accepted late data but only within watermark");
    Coder<String> utfCoder = StringUtf8Coder.of();
    TestStream<String> lettersStream = TestStream.create(utfCoder).addElements(
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
            TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT),
        ...
        .advanceWatermarkTo(FIRST_EVENT_INSTANT.plus(Duration.standardSeconds(15)))
        .addElements(TimestampedValue.of("a", ONE_SECOND_LATER_INSTANT))
        .advanceWatermarkToInfinity();
    Duration windowDuration = Duration.standardSeconds(5);
    Window<String> window = Window.<String>into(FixedWindows.of(windowDuration))
